...
describe Walk do
  ...
  it 'should find all files in directory tree' do
    files_count = 0
    Walk.walk(@test_path) do |_path, _subdirs, files|
  ...
  it 'should find all directories in directory tree' do
    dirs_count = 0
    Walk.walk(@test_path) do |_path, subdirs, _files|
  ...
  it 'should ignore disapearing entries' do
    expect do
      Walk.walk(@test_path) do |path, _subdirs, _files|
        remove_test_tree File.join(@test_path, 'subdir_2') if path == @test_path
      end
